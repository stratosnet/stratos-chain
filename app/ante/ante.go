...
func sigGasConsumer(
	meter sdk.GasMeter, _ []byte, pubkey crypto.PubKey, _ types.Params,
) error {
	switch pubkey.(type) {
...
// AccountSetupDecorator sets an account to state if it's not stored already. This only applies for MsgStratosTx.
...
// AnteHandle sets an account for MsgStratosTx (evm) if the sender is registered.
...
func (asd AccountSetupDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	for _, msg := range msgs {
		if msgStratosTx, ok := msg.(evmtypes.MsgStratosTx); ok {
			setupAccount(asd.ak, ctx, msgStratosTx.From)
		}
	}
