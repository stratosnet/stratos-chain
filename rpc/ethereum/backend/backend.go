...
func (e *EVMBackend) BaseFee(height int64) (*big.Int, error) {
	...
	resParams, err := e.queryClient.Params(types.ContextWithHeight(height), &evmtypes.QueryParamsRequest{})
	if err != nil {
		return nil, err
	}

	if resParams.Params.FeeMarketParams.NoBaseFee {
		return big.NewInt(0), nil
	}
	...
	res, err := e.queryClient.QueryBaseFee(types.ContextWithHeight(height), &evmtypes.QueryBaseFeeRequest{})
