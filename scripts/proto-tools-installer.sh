...
f_install_protoc_gen_gocosmos() {
    ...
    if ! grep "github.com/gogo/protobuf => github.com/regen-network/protobuf" go.mod &>/dev/null ; then
        echo -e "\tPlease run this command from somewhere inside the stratos folder."
        return 1
    fi

    go${GO_VERSION} get github.com/regen-network/cosmos-proto/protoc-gen-gocosmos 2>/dev/null
    f_print_done
}
...
f_install_protoc_gen_swagger() {
    ...
    pushd "${TEMPDIR}" >/dev/null
    go${GO_VERSION} install github.com/grpc-ecosystem/grpc-gateway/protoc-gen-swagger@latest
