...
func (k Keeper) CalcRewardForResourceNodeForTestnet(ctx sdk.Context, trafficList []types.SingleWalletVolume,
	distributeGoal types.DistributeGoal, rewardDetailMap map[string]types.Reward, totalNodesCnt sdk.Int,
	...
	for _, node := range resourceNodeList {
		...
		stakeRewardFromMiningPool := distributeGoal.BlockChainRewardToResourceNodeFromMiningPool.Amount.ToDec().Quo(totalNodesCnt.ToDec()).TruncateInt()
...
func (k Keeper) CalcRewardForIndexingNodeForTestnet(ctx sdk.Context, distributeGoal types.DistributeGoal,
	rewardDetailMap map[string]types.Reward, totalNodesCnt sdk.Int,
) (map[string]types.Reward, types.DistributeGoal) {
	...
	for _, node := range indexingNodeList {
		...
		stakeRewardFromMiningPool :=
			sdk.NewCoin(k.RewardDenom(ctx), distributeGoal.BlockChainRewardToIndexingNodeFromMiningPool.Amount.ToDec().Quo(totalNodesCnt.ToDec()).TruncateInt())
		...
		indexingRewardFromMiningPool :=
			sdk.NewCoin(k.RewardDenom(ctx), distributeGoal.MetaNodeRewardToIndexingNodeFromMiningPool.Amount.ToDec().Quo(indexingNodeCnt.ToDec()).TruncateInt())
		indexingRewardFromTrafficPool :=
			sdk.NewCoin(k.BondDenom(ctx), distributeGoal.MetaNodeRewardToIndexingNodeFromTrafficPool.Amount.ToDec().Quo(indexingNodeCnt.ToDec()).TruncateInt())
