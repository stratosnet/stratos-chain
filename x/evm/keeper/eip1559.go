...
func (k Keeper) CalculateBaseFee(ctx sdk.Context) *big.Int {
	...
	if !params.FeeMarketParams.IsBaseFeeEnabled(ctx.BlockHeight()) {
		return nil
	}
	...
	// governance).
	if ctx.BlockHeight() == params.FeeMarketParams.EnableHeight {
	...
	parentBaseFee := params.FeeMarketParams.BaseFee.BigInt()
	...
	parentGasTargetBig := new(big.Int).Div(gasLimit, new(big.Int).SetUint64(uint64(params.FeeMarketParams.ElasticityMultiplier)))
	...
	baseFeeChangeDenominator := new(big.Int).SetUint64(uint64(params.FeeMarketParams.BaseFeeChangeDenominator))
