package keeper

import (
	"fmt"
	"math"
	"math/big"

	sdk "github.com/cosmos/cosmos-sdk/types"
	sdkerrors "github.com/cosmos/cosmos-sdk/types/errors"

	"github.com/ethereum/go-ethereum/common"
	ethtypes "github.com/ethereum/go-ethereum/core/types"
	"github.com/ethereum/go-ethereum/core/vm"
	"github.com/ethereum/go-ethereum/crypto"
	"github.com/ethereum/go-ethereum/params"

	"github.com/stratosnet/stratos-chain/x/evm/statedb"
	"github.com/stratosnet/stratos-chain/x/evm/types"
)

var emptyCodeHash = crypto.Keccak256Hash(nil)

func (k *Keeper) createWithCustomAddress(ctx sdk.Context, msg ethtypes.Message, contractAddress common.Address, commit bool) (*common.Address, error) {
	cfg, err := k.EVMConfig(ctx)
	if err != nil {
		return nil, sdkerrors.Wrap(err, "failed to load evm config")
	}
	txConfig := statedb.NewEmptyTxConfig(common.BytesToHash(ctx.HeaderHash()))
	stateDB := statedb.New(ctx, k, txConfig)
	evm := k.NewEVM(ctx, msg, cfg, nil, stateDB)

	interpreter := vm.NewEVMInterpreter(evm, evm.Config)

	sender := vm.AccountRef(msg.From())

	stateDB.SetNonce(sender.Address(), msg.Nonce()+1)

	contractHash := evm.StateDB.GetCodeHash(contractAddress)
	if evm.StateDB.GetNonce(contractAddress) != 0 || (contractHash != (common.Hash{}) && contractHash != emptyCodeHash) {
		return nil, vm.ErrContractAddressCollision
	}

	snapshot := evm.StateDB.Snapshot()
	evm.StateDB.CreateAccount(contractAddress)
	evm.StateDB.SetNonce(contractAddress, 1)
	evm.Context.Transfer(evm.StateDB, msg.From(), contractAddress, msg.Value())

	contract := vm.NewContract(sender, vm.AccountRef(contractAddress), msg.Value(), msg.Gas())
	contract.SetCallCode(&contractAddress, common.Hash{}, msg.Data())

	ret, err := interpreter.Run(contract, nil, false)
	fmt.Printf("Ret of exec: %s\n", ret)
	if err != nil {
		return nil, err
	}

	// Check whether the max code size has been exceeded, assign err if the case.
	if err == nil && len(ret) > params.MaxCodeSize {
		err = vm.ErrMaxCodeSizeExceeded
	}

	// Reject code starting with 0xEF if EIP-3541 is enabled.
	if err == nil && len(ret) >= 1 && ret[0] == 0xEF {
		err = vm.ErrInvalidCode
	}

	// if the contract creation ran successfully and no errors were returned
	// calculate the gas required to store the code. If the code could not
	// be stored due to not enough gas set an error and let it be handled
	// by the error checking condition below.
	if err == nil {
		createDataGas := uint64(len(ret)) * params.CreateDataGas
		if contract.UseGas(createDataGas) {
			evm.StateDB.SetCode(contractAddress, ret)
		} else {
			err = vm.ErrCodeStoreOutOfGas
		}
	}

	// When an error was returned by the EVM or when setting the creation code
	// above we revert to the snapshot and consume any gas remaining. Additionally
	// when we're in homestead this also counts for code storage gas errors.
	if err != nil {
		evm.StateDB.RevertToSnapshot(snapshot)
		if err != vm.ErrExecutionReverted {
			contract.UseGas(contract.Gas)
		}
	}

	if commit {
		if err := stateDB.Commit(); err != nil {
			return nil, err
		}
	}
	return &contractAddress, nil
}

func (k *Keeper) getOrCreateEOA(ctx sdk.Context, addr common.Address) *statedb.Account {
	acc := k.GetAccount(ctx, addr)
	if acc == nil {
		acc = statedb.NewEmptyAccount()
		k.SetAccount(ctx, addr, *acc)
		acc = k.GetAccount(ctx, addr)
	}
	return acc
}

func (k *Keeper) getOrCreateContract(ctx sdk.Context, owner, contractAddress common.Address, data []byte) (*statedb.Account, error) {
	contractAcc := k.GetAccount(ctx, contractAddress)
	if contractAcc != nil {
		return contractAcc, nil
	}

	ownerAcc := k.GetAccount(ctx, owner)
	if ownerAcc == nil {
		return nil, fmt.Errorf("account '%s' not created, it should be issued first", owner)
	}

	msg := ethtypes.NewMessage(
		owner,
		nil,
		ownerAcc.Nonce,
		big.NewInt(0),
		uint64(math.MaxUint64),
		big.NewInt(0),
		big.NewInt(0),
		big.NewInt(0),
		data,
		ethtypes.AccessList{},
		false,
	)

	if _, err := k.createWithCustomAddress(ctx, msg, contractAddress, true); err != nil {
		return nil, err
	}

	contractAcc = k.GetAccount(ctx, contractAddress)
	// impossible, but in case of something
	if contractAcc == nil {
		panic("Contract not created, some indexdb issue or wrong code implementation")
	}
	return contractAcc, nil
}

// ExecuteProxyFunc execute provided function to a proxy contract
func (k *Keeper) UpdateProxyImplementation(ctx sdk.Context, p *types.UpdateImplmentationProposal) error {
	params := k.GetParams(ctx)
	consensusDeployerAddress := common.HexToAddress(params.ProxyProposalParams.ConsensusDeployerAddress)
	proxyAdminAddress := common.HexToAddress(params.ProxyProposalParams.ProxyAdminAddress)

	// TODO: PROXY: Add validation for this address
	proxyAddress := common.HexToAddress(p.ProxyAddress)
	implAddress := common.HexToAddress(p.ImplementationAddress)

	cdaAcc := k.getOrCreateEOA(ctx, consensusDeployerAddress)
	if cdaAcc == nil {
		return fmt.Errorf("failed to get consensus deployer acc for '%s' address", consensusDeployerAddress)
	}

	proxyAdminConstructorData := common.FromHex(types.ProxyAdminBin)
	_, err := k.getOrCreateContract(ctx, consensusDeployerAddress, proxyAdminAddress, proxyAdminConstructorData)
	if err != nil {
		return err
	}

	proxyConstructorData, err := types.EncodeContractFunc(
		types.TransparentUpgradableProxyABI,
		"",
		implAddress,
		proxyAdminAddress,
		[]byte{},
	)
	if err != nil {
		return err
	}

	fmt.Printf("consensusDeployerAddress: %s\n", consensusDeployerAddress.Hex())
	fmt.Printf("proxyConstructorData: %s\n", common.Bytes2Hex(proxyConstructorData))

	proxyConstructorData = append(common.FromHex(types.TransparentUpgradableProxyBin), proxyConstructorData...)

	_, err = k.getOrCreateContract(ctx, consensusDeployerAddress, proxyAddress, proxyConstructorData)
	if err != nil {
		return err
	}

	upgradeData, err := types.EncodeContractFunc(
		types.ProxyAdminABI,
		"upgradeAndCall",
		proxyAddress,
		implAddress,
		p.Data,
	)
	if err != nil {
		return err
	}

	msg := ethtypes.NewMessage(
		consensusDeployerAddress,
		&proxyAdminAddress,
		cdaAcc.Nonce,
		p.Amount.BigInt(),
		uint64(math.MaxUint64),
		big.NewInt(0),
		big.NewInt(0),
		big.NewInt(0),
		upgradeData,
		ethtypes.AccessList{},
		false,
	)

	cfg, err := k.EVMConfig(ctx)
	if err != nil {
		return sdkerrors.Wrap(err, "failed to load evm config")
	}
	txConfig := statedb.NewEmptyTxConfig(common.BytesToHash(ctx.HeaderHash()))
	stateDB := statedb.New(ctx, k, txConfig)
	evm := k.NewEVM(ctx, msg, cfg, nil, stateDB)

	sender := vm.AccountRef(msg.From())

	stateDB.SetNonce(sender.Address(), msg.Nonce()+1)
	_, _, vmErr := evm.Call(sender, *msg.To(), msg.Data(), msg.Gas(), msg.Value())
	if vmErr != nil {
		return vmErr
	}

	if err := stateDB.Commit(); err != nil {
		return err
	}

	return nil
}

// TODO: PROXY: Remove this, just for testing purposes before test writing
func (k *Keeper) TestDeployERC20Mock(ctx sdk.Context) (*common.Address, error) {
	params := k.GetParams(ctx)
	consensusDeployerAddress := common.HexToAddress(params.ProxyProposalParams.ConsensusDeployerAddress)

	cdaAcc := k.getOrCreateEOA(ctx, consensusDeployerAddress)
	if cdaAcc == nil {
		return nil, fmt.Errorf("failed to get consensus deployer acc for '%s' address", consensusDeployerAddress)
	}

	erc20Data := common.FromHex("")

	msg := ethtypes.NewMessage(
		consensusDeployerAddress,
		nil,
		cdaAcc.Nonce,
		big.NewInt(0),
		uint64(math.MaxUint64),
		big.NewInt(0),
		big.NewInt(0),
		big.NewInt(0),
		erc20Data,
		ethtypes.AccessList{},
		false,
	)

	cfg, err := k.EVMConfig(ctx)
	if err != nil {
		return nil, sdkerrors.Wrap(err, "failed to load evm config")
	}
	txConfig := statedb.NewEmptyTxConfig(common.BytesToHash(ctx.HeaderHash()))
	stateDB := statedb.New(ctx, k, txConfig)
	evm := k.NewEVM(ctx, msg, cfg, nil, stateDB)

	sender := vm.AccountRef(msg.From())

	stateDB.SetNonce(sender.Address(), msg.Nonce())
	_, deployedAddr, _, vmErr := evm.Create(sender, msg.Data(), msg.Gas(), msg.Value())
	if vmErr != nil {
		return nil, vmErr
	}

	if err := stateDB.Commit(); err != nil {
		return nil, err
	}

	return &deployedAddr, nil
}
