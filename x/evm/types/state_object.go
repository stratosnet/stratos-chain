...
type stateObject struct {
	code stratos.Code // contract bytecode, which gets set when code is loaded
...
func newStateObject(db *CommitStateDB, accProto authexported.Account) *stateObject {
	...
	// set empty code hash
	if stAccount.CodeHash == nil {
		stAccount.CodeHash = emptyCodeHash
	}

	return &stateObject{
		stateDB:                 db,
		account:                 stAccount,
		address:                 stAccount.EthAddress(),
...
func (so *stateObject) commitState() {
	...
	for _, state := range so.dirtyStorage {
		...
		if stratos.IsEmptyHash(state.Value) {
