...
func validateChainId(chainId *sdkmath.Int) error {
	if chainId.LTE(sdkmath.ZeroInt()) {
		return errors.Wrap(ErrInvalidChainConfig, "eip155 chain id should greater than 0")
	}
	return nil
}
...
func validateHash(hex string) error {
	if hex != "" && strings.TrimSpace(hex) == "" {
		return errors.Wrap(ErrInvalidChainConfig, "hash cannot be blank")
	}
